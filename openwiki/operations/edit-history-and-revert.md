---
type: operations
title: Edit History & Selective Revert
description: Navigating the audit log and executing safe two-step selective reverts (Refuse, Cascade, Reassign) in Theogony.
...
sources:
  - id: openwiki-source-f787a1aa2101e3e3a598d9b5
    resource: repo://theogony-db-sqlite/src/edit_log.rs
  - id: openwiki-source-5ef0075cec7a59f449cd3a7c
    resource: repo://theogony-db-sqlite/src/revert.rs
generated: { by: "openwiki/0.5.1", at: "2026-09-14T02:18:08.068Z" }
---

# Edit History & Selective Revert

Theogony records every data mutation in an immutable **Audit Log** (`theogony-db-sqlite/src/edit_log.rs`). Because genealogical research relies on verifiable evidence and rigorous attribution, the audit log allows researchers to inspect past modifications, audit provenance metadata, and perform **Selective Revert** actions to undo specific edits without discarding subsequent unrelated work.

---

## The Audit Log & Edit Architecture

Every write operation performed in the application—whether creating an individual, attaching a source citation, merging personas, or modifying a fact—is recorded as an atomic transaction action (`edit_actions`) composed of structured operations (`edit_ops`). 
...
- **Timestamp & Provenance**: Creation time, provenance metadata (`source: manual` or AI-assisted attribution), and the originating branch (`edit_branches`).
...
graph TD
    ...
    FindOps --> CheckConflicts["Run Pre-flight Conflict Analysis (`all_conflicts`)"]
    ...
    PromptDecisions --> DecisionRefuse["**Refuse**<br/>Abort revert for this branch / conflict"]
    PromptDecisions --> DecisionCascade["**Cascade**<br/>Automatically revert blocking actions newer first"]
    PromptDecisions --> DecisionReassign["**Reassign**<br/>Point dependent foreign key to alternative target"]
    
    DecisionRefuse --> Abort["Halt Revert (Return Unresolved Conflicts)"]
    ...
    ApplyRevert --> Success["Revert Complete & Recorded (`reverted_by` set)"]
```
...
2. **Cascade**:
   - **Behavior**: Recursively reverts the blocking downstream actions in reverse chronological order (newest first) before executing the primary revert.
...
3. **Reassign**:
   - **Behavior**: Remaps dependent foreign keys or field references to point to an alternative valid target record, allowing the primary revert to proceed without destroying unrelated downstream modifications.
...
- **Database Backups**: Always create a `.theb` database backup or export a portability package before performing complex multi-action reverts or branch merges.
